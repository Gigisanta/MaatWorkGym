...
async function seedEcommerce() {
  ...
  return { categories: 8, products: productCount, coupons: 3, configs: configs.length };
}
  ...
  try {
    ...
    if (existing > 0) {
      return NextResponse.json({ 
        ...
        stats: { categories: await prisma.ecommerceCategory.count(), products: existing } 
      });
    }
    const stats = await seedEcommerce();
    return NextResponse.json({ success: true, message: 'E-commerce seed completado', stats });
  } catch (error: any) {
...
export async function GET() {
  try {
    ...
    const categories = await prisma.ecommerceCategory.count();
    const coupons = await prisma.coupon.count();
    return NextResponse.json({ 
      hasData: products > 0,
      stats: { categories, products, coupons }
    });
  } catch (error: any) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
