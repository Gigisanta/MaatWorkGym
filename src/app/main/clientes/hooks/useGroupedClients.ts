...
export function useGroupedClients(
  ...
  const groupedData = useMemo((): GroupedData => {
    ...
    sortedSocios.forEach((socio) => {
      ...
      grouped.estado[
        status === 'vigente' ? 'vigentes' : status === 'porVencer' ? 'porVencer' : 'vencidas'
      ].push(socio);
    });
      ...
      case 'grupoMuscular':
        return Object.entries(groupedData.grupoMuscular).map(([grupo, items]) => ({
          ...
          label:
            grupo === 'sin_asignar'
              ? 'Sin asignar'
              : GRUPO_MUSCULAR_LABELS[grupo as keyof typeof GRUPO_MUSCULAR_LABELS] || grupo,
          subtitle: undefined,
          items,
        }));
      case 'estado':
        return [
          {
            ...
            items: groupedData.estado.vigentes,
          },
          {
            key: 'porVencer',
            label: 'Por vencer',
            subtitle: '7 días o menos',
            items: groupedData.estado.porVencer,
          },
          {
            key: 'vencidas',
            label: 'Vencidas',
            subtitle: 'Membresía vencida',
            items: groupedData.estado.vencidas,
          },
        ];
      default:
        return [];
